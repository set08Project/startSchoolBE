...
function processParagraph(paraXml) {
    ...
    paraXml = paraXml.replace(/<w:r[^>]*><w:rPr[^>]*><w:vertAlign w:val="subscript"\/><\/w:rPr>.*?<w:t[^>]*>([^<]+?)<\/w:t>/g, (_, text) => toUnicodeSubscript(text));
    paraXml = paraXml.replace(/<w:r[^>]*><w:rPr[^>]*><w:vertAlign w:val="superscript"\/><\/w:rPr>.*?<w:t[^>]*>([^<]+?)<\/w:t>/g, (_, text) => toUnicodeSuperscript(text));
    const elements = [];
    // Text runs

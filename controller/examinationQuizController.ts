...
function processParagraph(paraXml: string): string {
  ...
  while ((match = textRunRegex.exec(paraXml)) !== null) {
    ...
    if (match[0].includes("<m:oMath") || match[0].includes("<w:vertAlign")) continue;
...
const validateQuestion = (q: any, i: number) => {
  const errors: string[] = [];
  if (!q.question?.trim()) errors.push(`Question ${i + 1}: Missing text`);
  if (!q.options || q.options.length < 2)
    errors.push(`Question ${i + 1}: Need ≥2 options`);
  if (!q.answer?.trim()) errors.push(`Question ${i + 1}: Missing answer`);
  return { valid: errors.length === 0, errors };
};

export const createSubjectExamination = async (req: any, res: Response) => {
  ...
  try {
    ...
    const missing: any = {};
    if (!req.file) missing.file = true;
    if (!classID) missing.classID = true;
    if (!subjectID) missing.subjectID = true;
    if (!duration) missing.duration = true;
    if (!mark) missing.mark = true;
    ...
    if (ext === ".doc") {
      console.error(
        "Legacy .doc format uploaded — please convert to .docx or CSV."
      );
    ...
    if (questions.length === 0) {
      console.error(
        "No questions parsed — returning error. First 300 chars of raw: ",
        (await extractRawTextFromDocx(uploadedPath)).slice(0, 300)
      );
      return res.status(400).json({
